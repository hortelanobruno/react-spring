...
function addCMTS() {
	modalForTable('cmtss', 'New CMTS Manager', 4, 'IP, Port, Vendor, IPDR&nbsp;Enabled', 4);
}
function editCMTS() {
	modalForTableEdit('cmtss', 'Edit CMTS Manager', 4, 'IP, Port, Vendor, IPDR&nbsp;Enabled', 4);
}
...
function validatorVariablesOthers(RMIIP, RMIPort, RMIService, RestIP, RestPort, SepIP, SepPort, SERIP, SERPort) {
	...
	// si alguno falla no se realiza la llamada post
	if (!bolleanRMIIP || !bolleanRMIPort || !bolleanRMIService || !bolleanRestIP || !bolleanRestPort || !bolleanSepIP || !bolleanSepPort || !bolleanSERIP || !bolleanSERPort) {
		bollean = false;
	}
	return bollean;
}
